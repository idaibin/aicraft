...
| Project-class alignment | Preserves framework-native routing and applies repository-defined toolchain, script, directory, and naming rules without incidental upgrades. | Forces one SPA layout across Next.js, Astro, Tauri, and other project classes. |
...
| Existing implementation search | Consumes a current `code-context` inventory or performs targeted file/symbol search across relevant pages, routes, layouts, components, hooks, services, shared UI, tests, and exports. | Starts coding from a guessed path or creates a file before checking existing code. |
...
| New-file placement | Explains why existing candidates are insufficient and follows the current directory, naming, props, state, styling, export, and test conventions. | Creates a parallel `shared`, `common`, `ui`, `components`, hook, or service layer. |
...
| CSS declaration ownership | Keeps each spacing, sizing, alignment, typography, color, border, and overflow responsibility in one selector, token, variant, or prop. | Leaves duplicate or shadowing declarations in base, component, utility, and override layers. |
