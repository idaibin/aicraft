...
| Feedback loop | Names an already-run agent-runnable command, request, browser/client evidence loop, replay, or harness that goes red/green on the exact symptom, or stops with the exact missing evidence. | Proposes remediation or root-cause hypotheses before a red-capable loop exists. |
