...
7. Build a redacted `review-package.md` when durable or multipart context is needed; compact requests may be sent as inspected text. State facts, questions, selected theme/capability, evidence, exclusions, response contract, and Worktree/Git fingerprint or decision-basis identity without seeding conclusions. Package-only stops here.
...
- Keep outbound `review-package.md` separate from inbound local-private `review.md`.
