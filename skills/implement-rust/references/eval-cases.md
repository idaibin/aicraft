...
| Risk-layered validation | Adds relevant release, cross-target, Miri, sanitizer/leak, fuzz, stress, or repeated-operation gates and verifies tests ran. | Runs only `cargo check` for unsafe, FFI, parser, platform, concurrency, or resource-lifetime changes. |
...
expectations are correct and every quality case scores at least 7.
