...
Use these cases when changing `code-review` triggers, ownership labels, staging plans, review depth, specialist subreviews, repository-review boundary, contract-review scope, or metadata.

## Trigger Eval

| User prompt | Expected result | Why |
| --- | --- | --- |
| `Review all local changes and split commits.` | Should trigger Standard `code-review`. | Full dirty-tree review. |
...
| `Review the API contract chain in my uncommitted changes; check fields from backend to page.` | Should trigger `code-review`. | Local contract-chain review. |
| `Review this local module deletion and confirm manifests, exports, CI, docs, and indexes are clean.` | Should trigger `code-review`. | Structural completeness of current changes. |
...
| `Independently review main..feature and return P0-P3 findings.` | Should prefer `repo-review`. | Immutable branch/range review. |
| `Review PR 42 without posting comments.` | Should prefer `repo-review`. | Pull-request assessment, not local dirty-tree readiness. |
| `Review this entire repository at one commit for architecture, security, CI, and documentation defects.` | Should prefer `repo-review`. | Whole-repository snapshot review. |
| `Split this requirement into executable tasks.` | Should prefer `code-planner`. | Forward planning. |
| `Review this endpoint for token leakage and authorization risk.` | Should prefer `audit-security` after the target surface is clear. | Security-only audit. |
...
| `Audit this Rust service for concurrency, memory, and SQLite risks without a Git change set.` | Should prefer `audit-rust`. | Domain-wide read-only audit. |
| `Audit this frontend architecture for reuse, state, accessibility, and performance without a Git change set.` | Should prefer `audit-frontend`. | Domain-wide read-only audit. |

## Quality Eval

| Case | Expected evidence | Reject if |
| --- | --- | --- |
| Review-object boundary | Identifies the current local worktree/index as the primary object and routes repository grounding to `repo-context`, immutable repository/range/PR/release/package review to `repo-review`, and bounded security-only audits to `audit-security`. | Acts as a generic repository-review entrypoint. |
...
| Specialist composition | Keeps `code-review` as local Git-change coordinator while requesting bounded `audit-frontend`, `audit-rust`, or `audit-security` subreviews only for changed surfaces. | Sends the whole review workflow to an audit skill or skips required domain evidence. |
| Read-only review boundary | Leaves worktree files and Git state unchanged, runs only non-mutating checks, and proposes exact fix and delivery instructions. | Edits/creates/deletes/renames/formats files, runs write-mode checks, stages, commits, pushes, or rewrites refs. |
...
| Coverage honesty | States that conclusions apply to the local change set and relevant contract context, not the entire repository or release candidate. | Claims repository-wide safety from dirty-tree review. |
